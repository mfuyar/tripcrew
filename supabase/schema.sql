...
CREATE INDEX IF NOT EXISTS community_spots_created_at_idx ON community_spots(created_at);
CREATE INDEX IF NOT EXISTS community_spots_lat_lng_idx ON community_spots(latitude, longitude);
CREATE TRIGGER community_spots_updated_at BEFORE UPDATE ON community_spots
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TABLE IF NOT EXISTS community_spot_comments (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  spot_id     UUID NOT NULL REFERENCES community_spots(id) ON DELETE CASCADE,
  user_id     UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  content     TEXT NOT NULL,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS community_spot_comments_spot_id_idx ON community_spot_comments(spot_id);

CREATE TABLE IF NOT EXISTS community_spot_votes (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  spot_id     UUID NOT NULL REFERENCES community_spots(id) ON DELETE CASCADE,
  user_id     UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS community_spot_votes_spot_id_idx ON community_spot_votes(spot_id);

CREATE OR REPLACE FUNCTION update_community_spot_comment_count()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS community_spot_comment_count ON community_spot_comments;
CREATE TRIGGER community_spot_comment_count
  AFTER INSERT OR DELETE ON community_spot_comments
  FOR EACH ROW EXECUTE FUNCTION update_community_spot_comment_count();

CREATE OR REPLACE FUNCTION toggle_community_spot_vote(p_spot_id UUID, p_user_id UUID)
RETURNS community_spots AS $$
DECLARE
  result community_spots;
BEGIN
  IF p_user_id <> auth.uid() THEN
    RAISE EXCEPTION 'Cannot vote for another user';
  END IF;
  ...
  IF EXISTS (
    SELECT 1 FROM community_spot_votes
    WHERE spot_id = p_spot_id AND user_id = p_user_id
  ) THEN
    DELETE FROM community_spot_votes
    WHERE spot_id = p_spot_id AND user_id = p_user_id;
  ...
  ELSE
    INSERT INTO community_spot_votes (spot_id, user_id)
...
CREATE OR REPLACE FUNCTION get_nearby_community_spots(
...
GRANT EXECUTE ON FUNCTION toggle_community_spot_vote(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_nearby_community_spots(DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, UUID, INTEGER) TO authenticated;
