-- Users can be pre-added to a trip when an organizer invites them by email.
-- If that invited user later enters the invite code, treat it as success
-- instead of raising a duplicate/already-member error.
...
BEGIN
  ...
  IF EXISTS (
    ...
    INSERT INTO public.trip_join_requests (trip_id, user_id, status, reviewed_at)
    VALUES (target_trip.id, p_user_id, 'approved', NOW())
    ON CONFLICT (trip_id, user_id) DO UPDATE
    SET status = 'approved',
        reviewed_at = COALESCE(public.trip_join_requests.reviewed_at, NOW())
